"""Frozen encoders: images -> CNN features, text -> sentence embeddings.

Both are pretrained and never trained here, so running them over the whole
dataset before cross-validation introduces NO leakage: neither is fitted on
this data. The only thing fitted on data is the PCA, and that lives inside
the fold loop (see latent.py).

Both outputs are cached to disk, so the 10 folds reuse one extraction pass.
"""
...
def text_embeddings(texts: list[str], cfg: Config) -> np.ndarray:
    """(n_unique, 384) sentence embeddings for the unique strings. Cached."""
...
def text_matrix(samples: list[Sample], cfg: Config) -> tuple[np.ndarray, list[str]]:
    """Per-sample (N, 384) embedding matrix, plus the unique-string vocabulary."""
